...
import {
  ...
  query,
  where,
  serverTimestamp,arrayUnion,updateDoc,doc
...
const pasifTarihEkle=async (birim,tarih,deger,text)=>{
  ...
  if(docSnap.empty){
      ...
      docSnap.docs.forEach(async (belge)=>{
...
const randevuOlustur = async (veri) => {
  ...
  try {
    ...
    return `Randevunuz tarih: ${veri.tarih} saat: ${veri.saat} olarak ${veri.birim.ad} oluşturulmuştur.`;
  } catch (error) {
    return `Bir hata oluştu ${error.message}. Randevunuz eklenemedi`;
  }
};
